...
void play (cell_t ** board, cell_t ** newboard, int size, structures::ArrayList<std::pair<int,int>>* old_live_cells
  ...
  for (i=1; i<old_live_cells->size(); i++){
      auto p = old_live_cells->at(i);
      int i = p.first;
      int j = p.second;
        int old_state = board[i][j];
        int new_state = next_state[old_state];
        newboard[i][j] = new_state;
...
int main(int argc, char**argv){
  ...
  int i;
  //print(prev,size);
  auto old_live_cells = new structures::ArrayList<std::pair<int,int>>{(size_t)size*size*8};
  auto new_live_cells = new structures::ArrayList<std::pair<int,int>>{(size_t)size*size*8};
